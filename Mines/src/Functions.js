...
const getNeighbors = (board, row, column) => {
    ...
    rows.forEach(r => {
        columns.forEach(c => {
            const diferent = r !== row || c !== columns
...
const openField = (board, row, column) => {
    ...
    if (!field.opened) {
        ...
        if (field.mined) {
            ...
            const neightbor = getNeighbors(board, row, column)
            field.nearMines = neightbor.filter(n => n.mined).length
        }
